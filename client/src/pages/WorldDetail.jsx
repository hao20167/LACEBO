              ...
              <div>
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button
                    ...
                    className="bg-white border border-slate-300 hover:bg-slate-50 text-slate-700 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors"
                  >
                    + Propose
                  </button>
                </div>
                ...
                <div className="space-y-2">
                  {closedEvents.map((event) => (
                    <Link
                      key={event.id}
                      to={`/events/${event.id}`}
                      className="flex items-center gap-4 bg-white border border-slate-200 rounded-xl px-4 py-3 hover:border-slate-300 hover:shadow-sm transition-all opacity-70 hover:opacity-100"
                    >
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-0.5">
                          <span
                            className={`text-xs font-medium px-1.5 py-0.5 rounded border ${event.event_type === 'big' ? 'bg-indigo-50 text-indigo-600 border-indigo-100' : 'bg-slate-100 text-slate-500 border-slate-200'}`}
                          >
                            {event.event_type.toUpperCase()}
                          </span>
                        </div>
          ...
          <div>
            ...
            {showAnnForm && (
              <form
                ...
                className="bg-white border border-slate-200 rounded-xl p-5 mb-5 space-y-3 shadow-sm"
              >
                <input
                  type="text"
                ...
                <table className="w-full">
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-200 text-slate-500 text-xs uppercase tracking-wide">
                      ...
                      <th className="text-right px-4 py-3 w-28">Credits</th>
                    </tr>
                  </thead>
                  <tbody>
                    {leaderboard.map((m, i) => (
