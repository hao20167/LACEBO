...
import {
  ...
  eventParamsValidators,
} from '../middleware/validators/events.js';
...
router.get(
  '/world/:worldId/proposed',
  authMiddleware,
  validate(eventParamsValidators),
  (req, res) => {
  ...
  (req, res) => {
    ...
    if (member.role === 'dev') {
      ...
      if (event_type === 'big')
        return res
          .status(403)
          .json({ error: 'Only devs can create big events' });
      status = 'proposed';
    }
...
router.patch(
  ...
  (req, res) => {
    ...
    if (start_date)
      db.prepare('UPDATE events SET start_date = ? WHERE id = ?').run(
        start_date,
        event.id,
      );
    if (end_date)
